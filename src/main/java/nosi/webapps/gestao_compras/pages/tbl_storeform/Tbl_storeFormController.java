...
package nosi.webapps.gestao_compras.pages.tbl_storeform;
...
public class Tbl_storeFormController extends Controller {
	...
	public Response actionIndex() throws IOException, IllegalArgumentException, IllegalAccessException{
		...
		QueryInterface query = Core.query("gestao_compras_postgresql_1","SELECT id as id,description as description,location as location,id_user as id_user FROM public.tbl_store")
				.where("id=:id")
	...
	public Response actionList() throws IOException, IllegalArgumentException, IllegalAccessException{
		...
		/*----#end-code----*/
		return this.redirect("gestao_compras","Tbl_storeList","index");
	}
	...
	public Response actionSave() throws IOException, IllegalArgumentException, IllegalAccessException{
		...
		if(Core.isNull(isEdit)){
			r = Core.insert("gestao_compras_postgresql_1","public","tbl_store")		
				.addString("description",model.getDescription())
				.addString("location",model.getLocation())
				.addInt("id_user",model.getId_user())
				.execute();
		}else{
			r = Core.update("gestao_compras_postgresql_1","public","tbl_store")		
				.addString("description",model.getDescription())
				.addString("location",model.getLocation())
				.addInt("id_user",model.getId_user())
				.where("id=:id")
		...
		if(!r.hasError()){
			...
			Core.setMessageError();
			 return this.forward("gestao_compras","Tbl_storeForm","index");
		}
		/*----#end-code----*/
		return this.redirect("gestao_compras","Tbl_storeForm","index");
	}
}
